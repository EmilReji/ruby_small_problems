...
create array of n switches; fill with boolean false indicating each switch is off
run a loop n number of times
- use current num to determine step quantity
- use step quantity within inner loop; iterate over array of switches and change value using step
once final array of switches is created, create a new array to be returned by method
iterate over final array and for each true element, add that index + 1 to final array; then return final array
=end
